...
void doScanningOperation(GtkWidget *noUsed, char *devName) {
    ...
    FILE *file;
    gdouble fraction = 0;
    int c=0, totpix=0, q=0, hlp=0, resolution=0, paramSetRet=0, pages=0,
        scan_bpl=0L, scan_ppl=0L, scan_lines=0, lastInserted=0, shoulddoocr=0;
#ifdef CAN_OCR
    unsigned char *pic=NULL;
    int i=0, to=0, pageCount=0;
#endif // CAN_OCR //
    ...
    gtk_widget_set_sensitive(widget, FALSE);
    pageCount = gtk_spin_button_get_value(widget);
            ...
            if (status != SANE_STATUS_GOOD)
                ...
                if (status == SANE_STATUS_EOF)
                    ...
                    if(pages > pageCount)
                        ...
                        userMessage(g_strconcat("Insert page ", itoa(pages, 10), NULL), GTK_MESSAGE_QUESTION);
    ...
    if(shoulddoocr)
        ...
        if(scan_bpl && scan_ppl && scan_lines)
            ...
            ocrText = runocr((const char*)OCR_LANG_BRITISH, (const unsigned char*)pic, 1, scan_bpl, scan_ppl, scan_lines*pageCount);
    ...
    sql = g_strconcat(sql, 
                    ...
                    itoa(scan_lines, 10), ", ",
                    itoa(scan_ppl, 10), ", ", 
                    itoa(resolution/q, 10), ", ", 
                    itoa(pageCount, 10), ", '",dateStr, "', ", 
...
extern void startAquireOperation(void) {
    ...
    instructionText = "Use this screen to add a new document to your arhive. \n";
#ifdef CAN_OCR
    instructionText = g_strconcat(instructionText, 
                        "The first tab is to select a ODF document from disk. \n",
                        NULL);
#endif // CAN_READODF //
#ifdef CAN_SCAN
    instructionText = g_strconcat(instructionText, 
                        "Any additional tabs are connected to various SANE input devices.\n",
                        NULL);
#endif // CAN_SCAN //
