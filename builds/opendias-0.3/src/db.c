...
void open_db () {
    ...
    int rc;
    rc = sqlite3_open(g_strconcat(BASE_DIR, "openDIAS.sqlite3", NULL), &DBH);
    if ( rc )
        {
...
extern void connect_db () {
    ...
    if(g_file_test(g_strconcat(BASE_DIR, "openDIAS.sqlite3", NULL), G_FILE_TEST_EXISTS))
        {
        open_db ();
    ...
    for(i=version+1 ; i <= DB_VERSION ; i++)
        ...
        if(load_file_to_memory(g_strconcat(pa,"/../share/opendias/openDIAS.sqlite3.dmp.v", itoa(i, 10), ".sql", NULL), &sql))
            {
    ...
    do
        ...
        switch(type)
            ...
            case DB_NULL:
                ...
                debug_message("added null", DEBUGM);
                break;
            case DB_TEXT:
                sqlite3_bind_text(stmt, col, tmpList->data, strlen(tmpList->data), SQLITE_TRANSIENT );
                debug_message(g_strconcat("added str: ", tmpList->data ,NULL), DEBUGM);
                break;
            case DB_INT:
                sqlite3_bind_int(stmt, col, GPOINTER_TO_INT(tmpList->data));
                debug_message(g_strconcat("added int: ", itoa(GPOINTER_TO_INT(tmpList->data), 10) ,NULL), DEBUGM);
                break;
/*            case DB_DOUBLE:
    ...
    debug_message("running...", DEBUGM);
    rc = sqlite3_step(stmt);
    if( rc != SQLITE_DONE )
        {
        debug_message(g_strconcat("The following SQL gave an error: \n", sql, "\nThe resturn code was: ", itoa(rc, 10), "\nThe error was: ", sqlite3_errmsg(DBH), NULL), ERROR);
        }
