...
void doScanningOperation(GtkWidget *noUsed, char *devName) {
    ...
    FILE *file;
    gdouble fraction = 0;
    int c=0, totpix=0, i=0, to=0, q=0, hlp=0, resolution=0, paramSetRet=0,
...
extern void startAquireOperation(void) {
    ...
    GtkWidget *window, *vbox, *lab, *notebook, *fileSelector, 
            *button, *table, *box, *resolutionBar, *progress,
            *estQuality;
    SANE_Status status;
    SANE_Handle *openDeviceHandle;
    ...
    fileSelector = gtk_file_chooser_widget_new (GTK_FILE_CHOOSER_ACTION_OPEN);
    lab = gtk_label_new ("From: File");
    gtk_notebook_append_page(GTK_NOTEBOOK(notebook), fileSelector, lab);
    ...
    if(scanOK)
        ...
        for (i=0;device_list[i] && device_list[i]->model && device_list[i]->vendor && device_list[i]->name; i++)
            ...
            gtk_table_attach (GTK_TABLE (table), lab, 0, 2, 1, 2, GTK_EXPAND, GTK_EXPAND, 0, 0);
            ...
            gtk_table_attach (GTK_TABLE (table), lab, 0, 2, 2, 3, GTK_EXPAND, GTK_EXPAND, 0, 0);
            ...
            gtk_table_attach (GTK_TABLE (table), lab, 5, 6, 2, 3, GTK_EXPAND, GTK_EXPAND, 0, 0);
#ifdef HAVE_LIBTESSERACT_FULL
            ...
            gtk_table_attach (GTK_TABLE (table), lab, 0, 2, 3, 4, GTK_EXPAND, GTK_EXPAND, 0, 0);

            button = gtk_check_button_new();
            gtk_table_attach (GTK_TABLE (table), button, 2, 3, 3, 4, GTK_SHRINK, GTK_EXPAND, 0, 0);
            ...
            gtk_table_attach (GTK_TABLE (table), estQuality, 3, 8, 3, 4, GTK_SHRINK, GTK_EXPAND, 0, 0);
#endif // HAVE_LIBTESSERACT_FULL //
